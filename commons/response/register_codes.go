...
type ErrorMeta struct {
	StatusCode int    `json:"-"`       // HTTP状态码（不返回给客户端）,假如直接返回给客户端使用的话，用了“-”符号，则不会在body中的结构体中返回HTTP状态码,
...
func (coder ErrorMeta) HTTPStatus() int {
	if coder.StatusCode == 0 {
		return 500
	}
	return coder.StatusCode
}

// 为内部业务错误码绑定指定HTTP状态码以及对应Msg，创建特定ErrorMeta
func register(httpStatus int, message string) *ErrorMeta {
	if _, ok := allowedHTTPStatus[httpStatus]; !ok {
		panic("The HTTPStatusCode is not allowed!")
	}
...
func GetErrorMeta(code int) (*ErrorMeta, bool) {
	...
	// 返回副本，防止外部修改
	return &ErrorMeta{StatusCode: meta.StatusCode, Message: meta.Message}, true
}

// 自定义的业务状态码以及绑定的ErrorMeta
